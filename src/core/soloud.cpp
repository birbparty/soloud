...
//#define FLOATING_POINT_DEBUG
   ...
   !defined(WITH_CTRU_NDSP) && !defined(WITH_JACK) && !defined(WITH_NOSOUND) && !defined(WITH_MINIAUDIO)
...
namespace SoLoud
{
	...
	{		
		...
		if (!inited &&
			(aBackend == Soloud::CTRU_NDSP ||
			aBackend == Soloud::AUTO))
		{
			if (aBufferSize == Soloud::AUTO) buffersize = (samplerate * 120) / 1000;

			int ret = ctru_ndsp_init(this, aFlags, samplerate, buffersize, aChannels);
			if (ret == 0)
			{
				inited = 1;
				mBackendID = Soloud::CTRU_NDSP;
			}
